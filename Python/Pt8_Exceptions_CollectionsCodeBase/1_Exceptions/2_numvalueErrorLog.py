...
#different logging methods and severity --- the 'r' is rawstring and is supposed to ignore the path separaters and just find the path but it appears to only accept all following separaters after the pattern of the first one
logging.basicConfig(filename=r"Python/Pt8_Exceptions_CollectionsCodeBase/file2.txt", level=logging.DEBUG) 
